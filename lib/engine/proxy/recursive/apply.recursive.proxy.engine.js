    ...
    return (targetApply, thisArg, argumentList) => {
        ...
        switch (key) {
            ...
            case case_tool_enum_1.CaseToolEnum.BeforeAndConnectionWrapperAndMethod:
                ...
                break;
            case case_tool_enum_1.CaseToolEnum.ConnectionWrapperAndModel:
                // Connection
                result = lists.connectionWrapper.reduce((previousValue, currentValue) => {
                    // TODO use previousValue if will add new connection between method or models
                    return currentValue(...argumentList, lists.models.map(method => instanceof_method_1.InstanceofMethod), lists.models);
                }, result);
                break;
            case case_tool_enum_1.CaseToolEnum.ConnectionAndAfterWrapperAndModel:
                // Connection
                result = lists.connectionWrapper.reduce((previousValue, currentValue) => {
                    // TODO use previousValue if will add new connection between method or models
                    return currentValue(...argumentList, lists.models.map(method => instanceof_method_1.InstanceofMethod), lists.models);
                ...
                break;
            case case_tool_enum_1.CaseToolEnum.BeforeAndConnectionAndAfterWrapperAndModel:
                // Before
                result = lists.beforeWrapper.reduce((previousValue, currentValue) => {
                    // TODO lists.connectionWrapper make reduce
                    return currentValue(lists.connectionWrapper[0], argumentList, lists.models.map(method => instanceof_method_1.InstanceofMethod), lists.models);
                }, result);
                // After
                result = lists.afterWrapper.reduce((previousValue, currentValue) => {
                    return currentValue(previousValue);
                ...
                break;
            case case_tool_enum_1.CaseToolEnum.BeforeAndConnectionWrapperAndModel:
                // Before
                result = lists.beforeWrapper.reduce((previousValue, currentValue) => {
                    // TODO lists.connectionWrapper make reduce
                    return currentValue(lists.connectionWrapper[0], argumentList, lists.models.map(method => instanceof_method_1.InstanceofMethod), lists.models);

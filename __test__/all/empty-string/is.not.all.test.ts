...
describe('is.all.string.not.empty', () => {
    ...
    it.each(dataForBadTesting)('is.all.string.not.empty: Should true for $value', ({value}: {value: any}) => {
        expect(is.all.string.not.empty(value)).toBe(true);
    });

    it.each(advancedDataForBadTesting)('is.all.string.not.empty: Should true for $value', ({value}: {value: any}) => {
        expect(is.all.string.not.empty(value)).toBe(true);
    });

...
describe('is.all.array.not.empty', () => {
    ...
    it.each(dataForBadTesting)('is.all.array.not.empty: Should true for $value', ({value}: {value: any}) => {
        expect(is.all.array.not.empty(value)).toBe(true);
    });

    it.each(advancedDataForBadTesting)('is.all.array.not.empty: Should true for $value', ({value}: {value: any}) => {
        expect(is.all.array.not.empty(value)).toBe(true);
    });
